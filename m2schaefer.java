...
void keyPressed() {
  ...
  if (key == 'm') { mice(); }
  if (key == '1') { kingX= random(middle+100, right); kingY= random(top, bottom); }
  if (key == '2') { funX= random(middle+100, right); funY= random(top, bottom); }
  if (key == '3') { songX= random(middle+100, right); songY= random(top, bottom); }  
}

void mouseClicked() {
  if ( dist(kingX,kingY, mouseX,mouseY) < 15) { kingX= random(middle+100, right); kingY= random(top, bottom); }
  if ( dist(funX,funY, mouseX,mouseY) < 15)   { funX= random(middle+100, right); funY= random(top, bottom); }
  if ( dist(songX,songY, mouseX,mouseY) < 15) { songX= random(middle+100, right); songY= random(top, bottom); } 
}
...
void mice() {
  miceX += 5;
  fill(0,0,0);
  ellipse(miceX+85,height-75,20,20);
  ellipse(miceX+115,height-75,20,20);  
  ellipse(miceX+100,height-60, 30,30);
}
